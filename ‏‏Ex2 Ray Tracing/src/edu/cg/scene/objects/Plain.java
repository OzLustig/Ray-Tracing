...
public class Plain extends Shape {
    ...
    {
        Vec plainVector = new Vec(a, b, c);
        double dotProduct = ray.direction().dot(plainVector);
		double distance = this.substitute(ray.source());
		double scale = (- distance) / dotProduct;
		if((scale > Ops.epsilon) && (scale < Ops.infinity) )
		{
			Hit intersection = new Hit(scale, normal(ray));
			return intersection;
		}
		else
		{
    ...
    public double substitute(Point point) {
        Vec planeVector = new Vec(a, b, c);
        Vec pointVector = point.toVec();
		double dotProduct = planeVector.dot(pointVector);
		return (dotProduct + d);
    }
    ...
    public Vec normal(Ray ray) {
		...
		double dotProduct = rayDirection.dot(this.normal());
		if(dotProduct < 0) {
    		normalResult = normal();
    	}
    	else {
    		normalResult = normal().neg();
    	}
    	return normalResult;
    }
